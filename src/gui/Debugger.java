...
public class Debugger extends javax.swing.JFrame {
    ...
    public void setBpoints(){
        if(bBP1Checked){
         cpu.setBreakpoint(nBP1, true);
        }
        if(bBP2Checked){
         cpu.setBreakpoint(nBP2, true);
        }
        if(bBP3Checked){
         cpu.setBreakpoint(nBP3, true);
        }
        if(bBP4Checked){
         cpu.setBreakpoint(nBP4, true);
        }
        if(bBP5Checked){
         cpu.setBreakpoint(nBP5, true);
        }
    }
    ...
    public void fillAsmCode(){
        ...
        int memPtr = cpu.getRegPC();
        for(int j=0;j<10;j++){
        int memPtrTmp=memPtr;
        for(int i=memPtr;i<memPtr+5;i++){
           Z80Dis.Opcodes[i]= (0xff) & (byte)mem.readByte(memPtrTmp);
           memPtrTmp++;
        }
    ...
    public void fillStack() {
        ...
        for (int i = nSPAdr; i < nSPAdr + 10; i += 2) {
            ...
            String strLine = String.format("#%04X #%04X\n", i1, 256 * ((0xff) & (byte) mem.readByte(i2)) + ((0xff) & (byte) mem.readByte(i1)));
...
//        jTextStack.append(strA3);
    ...
    public void fillDataShow() {
        ...
        if (cf.bShowCode) {
            ...
            for (int i = 0; i < 7; i++) {
                ...
                for (int j = 0; j < 8; j++) {
                    int nBajt = (0xff) & (byte) mem.readByte(nMemAdr);
                    if ((nBajt >= 32) && (nBajt <= 128)) {
                        strAsci += (char) (nBajt);
                    } else {
                        strAsci += ".";
                    }
                    strOneLine += String.format("%02X ", (0xff) & (byte) mem.readByte(nMemAdr));
                    nMemAdr++;
                    if (nMemAdr == 65536) {
                        nMemAdr = 0;
                    }
    ...
    public void showDialog() {
        ...
        if(stpBP.bStatus){
         ...
         cpu.setBreakpoint(stpBP.nAdress, false);
         stpBP.bStatus=false;
         setBpoints();
    ...
    private void jStepIntoButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jStepIntoButtonActionPerformed
        cpu.setBreakpoint(cpu.getRegPC(),false);//vypnu pripadny breakpoint na aktualni adrese, jinak bych se nikam nepohnul
        cpu.setBreakpoint(stpBP.nAdress, true);
        stpBP.bStatus=true;
                            ...
                            if (txtByte != null) {
                                if (txtByte.getSaveNeeded()) {
                                    ...
                                    if (nByteAddress != null) {
                                        mem.writeByte(nByteAddress, nNewByte);
                                        nByteAddress = null;
                                        fillAsmCode();
                                        fillStack();
                                    }
